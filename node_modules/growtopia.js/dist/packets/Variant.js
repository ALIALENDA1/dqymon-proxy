...
const TankPacket_js_1 = require("./TankPacket.js");
...
class Variant {
    ...
    static toArray(data) {
        ...
        for (let i = 1; i <= count; i++) {
            ...
            const typeName = Constants_js_1.VariantTypes[type];
            pos += 1;
            switch (type) {
                case Constants_js_1.VariantTypes.STRING: {
                ...
                case Constants_js_1.VariantTypes.UNSIGNED_INT: {
                    const value = data.readUint32LE(pos);
                    pos += 4;
                    arr.push({ index, type, typeName, value });
                    break;
                }
                case Constants_js_1.VariantTypes.SIGNED_INT: {
                    const value = data.readInt32LE(pos);
                    pos += 4;
                    arr.push({ index, type, typeName, value });
                    break;
                }
                case Constants_js_1.VariantTypes.FLOAT_1: {
                    const value = [data.readFloatLE(pos)];
                    pos += 4;
                    arr.push({ index, type, typeName, value });
                    break;
                }
                case Constants_js_1.VariantTypes.FLOAT_2: {
                    let value = [];
                    for (let i = 1; i <= 2; i++) {
                ...
                case Constants_js_1.VariantTypes.FLOAT_3: {
                    let value = [];
                    for (let i = 1; i <= 3; i++) {
                        value.push(data.readFloatLE(pos));
                        pos += 4;
    ...
    parse() {
        ...
        this.args.forEach((arg) => {
            ...
            switch (typeof arg) {
                case "string": {
                    buf.push(Constants_js_1.VariantTypes.STRING);
                ...
                case "number": {
                    ...
                    if (arg < 0) {
                        ...
                        buf.push(Constants_js_1.VariantTypes.SIGNED_INT);
                    }
                    else {
                        bytes = new Uint32Array(1);
                        buf.push(Constants_js_1.VariantTypes.UNSIGNED_INT);
                    }
                    bytes[0] = arg;
                    const uint8_buf = new Uint8ClampedArray(bytes.buffer);
                    buf = [...buf, ...uint8_buf];
                ...
                case "object": {
                    if (!Array.isArray(arg))
                        return;
                    const type = Constants_js_1.VariantTypes[`FLOAT_${arg.length}`];
                    if (!type)
                        return;
                    buf.push(type);
                    arg.forEach((float) => {
        ...
        return TankPacket_js_1.TankPacket.from({
            type: 0x1,
